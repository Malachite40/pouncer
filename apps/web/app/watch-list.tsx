  ...
  const filteredWatches = useMemo(() => {
    ...
    if (sortKey) {
      result = [...result].sort((a, b) => {
        ...
        if (sortKey === "status") {
          cmp =
            getStatusRank(normalizeStatus(a.lastStockStatus), a.isActive) -
            getStatusRank(normalizeStatus(b.lastStockStatus), b.isActive);
        } else if (sortKey === "price") {
          const pa = a.lastPrice ? Number.parseFloat(a.lastPrice) : null;
          const pb = b.lastPrice ? Number.parseFloat(b.lastPrice) : null;
    ...
    return (
      <div className="rounded-lg border border-dashed border-border bg-card/96 p-8 text-center sm:p-12">
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">
          Add a product URL, define the signal you care about, and let Pounce
          keep watch.
        </p>
              ...
              <div className={`${boardColumnClassName} hidden lg:grid`}>
                ...
                <div className="space-y-1 lg:pr-3">
                  <div className="text-xs text-muted-foreground">
                    Last {formatLastChecked(watch.lastCheckedAt)}
                  </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex shrink-0 items-center gap-3">
                    <PriceHistoryChart
                      data={historyData}
                      variant="minimal"
                      className="h-6 w-24 shrink-0 sm:w-32 pointer-events-none"
                      mode={
                        watch.checkType === "stock"
                          ? "stock"
                          : watch.checkType === "both"
...
function StatusBadge({
  ...
  return (
    <span className={`${badgePillClassName} ${style.className}`}>
      <span className={`h-1.5 w-1.5 rounded-full ${style.pulseClassName}`} />
